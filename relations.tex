...
A static inspection on 17 September 2026 recovered the following arithmetic
from one native firing route.  This is a reconstruction, not an official
...
area, calibration distance, accuracy, and marksmanship are strongly supported
by record layout and accessors but have not been traced completely through
the generated database deserializer.  Some modifiers and branch predicates
remain unidentified.  The executable identity and function addresses are
recorded in Section~\ref{sec:native-provenance}; its correspondence to the
repository's patch/build baseline remains a provenance check, not an
assumption supplied by these equations.

The route has two distinct randomization stages: a target-local aim-point
offset and angular perturbations of the launch vector.  Other callers,
...
For distance $r\geq0.01$, the reconstructed precision factor is
\begin{align}
...
function returns $F=1$ before this calculation.  In particular, $F$ is
...
\begin{equation}
  ...
  \begin{cases}
    C, & n=0,\\
    \max\{C(1-0.1n),\,0.5C\}, & n>0.
  \end{cases}
\end{equation}
The counter's meaning is unresolved; it must not be called unit rank or
burst index without further evidence.  The subsequent arithmetic is
\begin{equation}
  S(r)=\frac{C_{\mathrm{eff}}}{F(r)},\qquad L(r)=\sqrt{S(r)}.
...
ordinary caller supplies $s_\alpha=1$.  The denominator is reconstructed as
$D=\max(|v_y|,r)$, where the loaded component is consistent with vertical
launch velocity.  This apparently mixed-unit expression is retained from
the instructions rather than silently replaced by $r$; its interpretation
needs further checking.  The inverse-tangent identification is supported by
the helper's range reduction and ballistic uses, not a separate runtime test.

With $M=V=1$, $\pi_{\mathrm{pen}}=0$, $\theta=0$, $n=0$, and $a+b>0$,
between the clamps in Equation~\ref{eq:missile-precision},
\begin{equation}
  S(r)=\frac{100C}{a+b}\left(\frac{r}{d}\right)^2.
...
Static inspection of \texttt{Warhammer3.exe}, 17 September 2026.
...
\end{quote}
The binary's patch/build correspondence requires independent confirmation.
...
record offsets \texttt{+0x88} and \texttt{+0x8c}, interpreted as calibration
distance and area.  Marksmanship is interpreted at record \texttt{+0x68}.
